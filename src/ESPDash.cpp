...
bool ESPDashClass::init(AsyncWebServer &server)
{
    if (!SPIFFS.begin()) {
        if (DEBUG_MODE) {
...
void ESPDashClass::updateNumberCard(const char *_id, int _value)
{
    for (int i = 0; i < NUMBER_CARD_LIMIT; i++) {
        if (number_card_id[i] == _id) {
            ...
            number_card_value[i] = _value;
#if ARDUINOJSON_VERSION_MAJOR == 6 && ARDUINOJSON_VERSION_MINOR == 7
            DynamicJsonDocument doc;
#elif ARDUINOJSON_VERSION_MAJOR == 6 && ARDUINOJSON_VERSION_MINOR == 8
...
#elif ARDUINOJSON_VERSION_MAJOR == 6 && ARDUINOJSON_VERSION_MINOR == 8
            DynamicJsonDocument carddoc(1024);
#endif
